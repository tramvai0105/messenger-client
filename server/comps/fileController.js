...
class fileController{
    async changeAvatar(req, res){
        ...
        if(user.avatar != "http://localhost:5000/images/elvis.jpg"){
            let avatarPath = user.avatar.split("http://localhost:5000")[1]
            fs.unlink(`.${avatarPath}`, (e)=>{
                if(e){console.log(e)}else{}
            })
        }
        user.avatar = "http://localhost:5000/" + "images/" + `${avatar.name}.png`
        await user.save();
    ...
    async getUserAvatar(req, res){
        ...
        if(!fs.existsSync(`.${avatarPath}`)){
            avatar = "http://localhost:5000/images/elvis.jpg"
            user.avatar = avatar;
            await user.save()
        }
    ...
    async getUsersAvatarsByNames(req, res){
        ...
        for(let i = 0; i < users.length; i++){
            ...
            let avatarPath = user.avatar.split("http://localhost:5000")[1]
            if(!fs.existsSync(`.${avatarPath}`)){
                user.avatar = "http://localhost:5000/images/elvis.jpg";
            }
            pair = [users[i], avatar];
            avatars.push(pair);
        }
